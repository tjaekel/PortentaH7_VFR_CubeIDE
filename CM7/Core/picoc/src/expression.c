...
extern struct ParseState *gTopParser;

long ExpressionCoerceInteger(struct Value *Val)
{
    if ( ! Val->Val)
    {
        //fatal error: can happen if uninitialized pointer used in Pico-C
        ProgramFail(gTopParser, "uninitialized pointer?");
    }

    //BUT: we cannot catch if other values are taken as pointers here
...
void ExpressionStackPushValueNode(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *ValueLoc)
{
    ...
    *StackTop = StackNode;
#ifdef FANCY_ERROR_MESSAGES	//XXXXX
...
void ExpressionAssignToPointer(struct ParseState *Parser, struct Value *ToValue, struct Value *FromValue, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
    ...
    else if (AllowPointerCoercion && IS_NUMERIC_COERCIBLE(FromValue))
    {
        ...
        ToValue->Val->NativePointer = (void *)/*(unsigned short)*/ExpressionCoerceUnsignedInteger(FromValue);
    }
    else
...
#if 0
            //not correct!!
            if ((DestValue->Typ->Base == TypeArray) && (SourceValue->Typ->Base == TypePointer))
            {
                //if array of char and a string assigned
                if (DestValue->Typ->FromType->Base != TypeChar)
                    ProgramFail(Parser, "array of char expected");
                if (SourceValue->Typ->FromType->Base != TypeChar)
                    ProgramFail(Parser, "string exptected");
                if ((strlen(SourceValue->Val->Identifier) + 1) > (size_t)(DestValue->Typ->Sizeof))
                    ProgramFail(Parser, "assigned string too large");

                memcpy((void *)DestValue->Val, (void *)SourceValue->Val->Identifier, strlen(SourceValue->Val->Identifier + 1));
            }
            else
#endif
            {
                //XXXX: ATT: it is possible to assign a complete array to another - not possible in C
                //and a bug here: we memcpy too much and destroy the Val!
                ...
                //DEBUG
                int i = TypeSizeValue(DestValue);
                if (i > (int)sizeof(DestValue->Val))
                    ProgramFail(Parser, "invalid assignment?");
                memcpy((void *)DestValue->Val, (void *)SourceValue->Val, /*TypeSizeValue(DestValue)*/ i);
            }
            break;
...
void ExpressionInfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *BottomValue, struct Value *TopValue)
{
    ...
    if (Op == TokenLeftSquareBracket)
    {
        ...
        {
            case TypeArray:   if (ArrayIndex >= BottomValue->Typ->ArraySize)
                              {
                                ProgramFail(Parser, "index %d out of array[%d]", ArrayIndex, BottomValue->Typ->ArraySize);
                              }
                              Result = VariableAllocValueFromExistingData(Parser, BottomValue->Typ->FromType, (union AnyValue *)(&BottomValue->Val->ArrayMem[0] + TypeSize(BottomValue->Typ->FromType, 0, TRUE) * ArrayIndex), BottomValue->IsLValue, BottomValue->LValueFrom);
                              break;
            case TypePointer: if (ArrayIndex >= BottomValue->Typ->FromType->DerivedTypeList->ArraySize)
                              {
                                ProgramFail(Parser, "index %d out of array[%d]", ArrayIndex, BottomValue->Typ->FromType->DerivedTypeList->ArraySize);
                              }
                              Result = VariableAllocValueFromExistingData(Parser, BottomValue->Typ->FromType, (union AnyValue *)((char *)BottomValue->Val->NativePointer + TypeSize(BottomValue->Typ->FromType, 0, TRUE) * ArrayIndex), BottomValue->IsLValue, BottomValue->LValueFrom);
                              break;
...
#if 0
                                            //we reuse as: return the larger value
                                            if (BottomInt > TopInt)
                                                ResultInt = BottomInt;
                                            else
                                                ResultInt = TopInt;
#else
                                            {
                                                struct Value *thirdVal;
                                                struct Value *LexValue;
                                                enum LexToken Token = LexGetToken(Parser, &LexValue, TRUE);
                                                if (Token != TokenColon)
                                                    ProgramFail(Parser, ": expected");
                                                ExpressionParse(Parser, &thirdVal);
                                                if (Parser->Mode == RunModeRun)
                                                    VariableStackPop(Parser, thirdVal);
                                                if (BottomInt)
                                                    ResultInt = TopInt;
                                                else
                                                {
                                                    if (thirdVal->Typ->Base == TypeChar)
                                                        ResultInt = (long)thirdVal->Val->Character;
                                                    else
                                                        ResultInt = thirdVal->Val->LongInteger;
                                                }
                                            }
#endif
                                            break;
            //TokenColon actually never seen here - we handle it in ExpressionParse()
...
#ifdef FANCY_ERROR_MESSAGES	//XXXX
    StackNode->Line = Parser->Line;
    StackNode->CharacterPos = Parser->CharacterPos;
#endif
#ifdef DEBUG_EXPRESSIONS
...
void ExpressionGetStructElement(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Token)
{
    ...
    if (Parser->Mode == RunModeRun)
    {
        ...
        if (StructType->DerivedTypeList)
        {
            if (StructType->DerivedTypeList->Base == TypeArray)
            {
                ProgramFail(Parser, "sorry: 'array of struct' not yet supported");
            }
        }
        //else
        {
            ...
            {
                //check if we have int.size(); - let's handle this like a function call
                //XXXX:
                if (strcmp(Ident->Val->Identifier, "size") == 0)
                {
                    struct Value *LexValue;
                    if (LexGetToken(Parser, &LexValue, FALSE) == TokenOpenBracket)
                    {
                        ...
                        if (LexGetToken(Parser, &LexValue, TRUE) == TokenCloseBracket)
                        {
                            ...
                            *StackTop = (*StackTop)->Next;
                            ...
                            //ExpressionPushInt(Parser, StackTop, 4);
                            if (ParamVal->Typ == &TypeType)
                                //ExpressionPushInt(Parser, StackTop, TypeSize(ParamVal->Val->Typ, ParamVal->Val->Typ->ArraySize, TRUE));
                                ExpressionPushInt(Parser, StackTop, ParamVal->Val->Typ->ArraySize);
                            else
                                //ExpressionPushInt(Parser, StackTop, TypeSize(ParamVal->Typ, ParamVal->Typ->ArraySize, TRUE));
                                ExpressionPushInt(Parser, StackTop, ParamVal->Typ->ArraySize ? ParamVal->Typ->ArraySize : ParamVal->Typ->ArraySize + 1);
                            return;
                        }
                        else
                            ProgramFail(Parser, "a ')' expected");
                    }
                    else
                        ProgramFail(Parser, "a '(' expected");
                }
        ...
        else if (Token == TokenIdentifier)
        {
            ...
            else
            {
                if (Parser->Mode == RunModeRun)
                {
                    ...
                    else if (VariableValue->Typ == (TValueType *)TypeVoid)
...
void ExpressionParseFunctionCall(struct ParseState *Parser, struct ExpressionStack **StackTop, const char *FuncName)
{
    ...
    if (Parser->Mode == RunModeRun)
    {
        ...
        //XXXX: ATT - it destroys FileName in FuncValue->FuncDef->Body !
        ExpressionStackPushValueByType(Parser, StackTop, FuncValue->Val->FuncDef.ReturnType);
        //FIX temporary
        FuncValue->Val->FuncDef.Body.FileName = FuncName;
#ifdef FANCY_ERROR_REPORTING
        FuncValue->Val->FuncDef.Body.SourceText = StrEmpty;
#endif
    ...
    ArgCount = 0;
    do {
        if (Parser->Mode == RunModeRun && ArgCount < FuncValue->Val->FuncDef.NumParams && FuncValue->Val->FuncDef.ParamType[ArgCount]->Sizeof != 0)
        ...
        if (ExpressionParse(Parser, &Param))
        {
            if (Parser->Mode == RunModeRun)
            {
                if (ArgCount < FuncValue->Val->FuncDef.NumParams && FuncValue->Val->FuncDef.ParamType[ArgCount]->Sizeof != 0)
                {
    ...
    if (Parser->Mode == RunModeRun)
    {
        ...
        if (FuncValue->Val->FuncDef.Intrinsic == NULL)
        {
            ...
            {
                if (FuncValue->Val->FuncDef.ParamType[Count]->Sizeof == 0)
                    break;
