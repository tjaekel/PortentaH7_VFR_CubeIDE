...
struct TableEntry *StringHashTable[STRING_TABLE_SIZE] = {0};
char *StrEmpty = NULL;
...
static struct TableEntry *TableSearch(struct Table *Tbl, const char *Key, int *AddAt)
{
    ...
    int HashValue = ((PTR_TYPE_INT)Key) % Tbl->Size;   /* shared strings have unique addresses so we don't need to hash them */
...
char *TableSetIdentifier(struct Table *Tbl, const char *Ident, int IdentLen)
{
    ...
    AddAt = 0;
    FoundEntry = TableSearchIdentifier(Tbl, Ident, IdentLen, &AddAt);
